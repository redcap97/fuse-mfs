...
static int lookup(ino_t root_nr, const char *path, struct fsdriver_node *node) {
  ...
  assert(fs_lookup(root_nr, ".", &cur_node, &is_mountpt) == OK);
...
static void setup(void) {
  ...
  if (device_path == NULL) {
    panic("missing device parameter");
  }
  glue_setup(device_path);
}
...
int main(int argc, char *argv[]) {
  ...
  if (fs_mount(dev, 0, &root_node, &res_flags) != OK) {
    panic("couldn't mount the device");
  }

  assert(fs_mountpt(root_node.fn_ino_nr) == OK);
